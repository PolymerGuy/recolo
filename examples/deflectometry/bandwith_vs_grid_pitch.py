...
# Test of the bandwidth of the grid method for different grid pitches. This is done by deforming a grid using a
# harmonic displacement field with increasing frequency and determining the peak amplitude. The peak amplitude
# measured by the method decreases as the frequency of the displacement field increases, see [1] for further
# discussion.
#
...
for grid_pitch in grid_pitches:
    ...
    for disp_period in disp_periodes:
        ...
        xs, ys, Xs_lagr, Ys_disp_lagr, u_x, u_y = recolo.artificial_grid_deformation.harmonic_disp_field(disp_amp,
                                                                                                        disp_period,
                                                                                                        disp_n_periodes,
                                                                                                        formulation="lagrangian")
        # Generate the grid images
        grid_undeformed = recolo.artificial_grid_deformation.dotted_grid(xs, ys, grid_pitch)
        grid_displaced_eulr = recolo.artificial_grid_deformation.dotted_grid(Xs_lagr, Ys_disp_lagr, grid_pitch)
    ...
    plt.plot(disp_periodes, theoretical_bias,'--', label="Theoretical bias for grid pitch: %i"%grid_pitch)
