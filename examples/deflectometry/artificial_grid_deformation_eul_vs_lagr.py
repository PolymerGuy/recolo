...
xs, ys, Xs_eulr, Ys_eulr, _, _ = recolo.artificial_grid_deformation.harmonic_disp_field(disp_amp, disp_period,
                                                                                           disp_n_periodes,
                                                                                           formulation="eulerian")

_, _, Xs_lagr, Ys_disp_lagr, u_x, u_y = recolo.artificial_grid_deformation.harmonic_disp_field(disp_amp,
                                                                                              disp_period,
                                                                                              disp_n_periodes,
                                                                                              formulation="lagrangian")

# Generate the grid images
grid_undeformed = recolo.artificial_grid_deformation.dotted_grid(xs, ys, grid_pitch, oversampling=oversampling,
                                                                pixel_size=1)

grid_deformed_eulr = recolo.artificial_grid_deformation.dotted_grid(Xs_eulr, Ys_eulr, grid_pitch,
                                                                   oversampling=oversampling,
                                                                   pixel_size=1)
grid_deformed_lagr = recolo.artificial_grid_deformation.dotted_grid(Xs_lagr, Ys_disp_lagr, grid_pitch,
                                                                   oversampling=oversampling,
                                                                   pixel_size=1)
# Calculate the phase fields
phase_x0, phase_y0 = recolo.deflectomerty.detect_phase(grid_undeformed, grid_pitch)
phase_x_eulr, phase_y_eulr = recolo.deflectomerty.detect_phase(grid_deformed_eulr, grid_pitch)
phase_x_lagr, phase_y_lagr = recolo.deflectomerty.detect_phase(grid_deformed_lagr, grid_pitch)

# Calculate the displacements from the phase fields
disp_x_eulr, _ = recolo.deflectomerty.disp_fields_from_phases(phase_x_eulr, phase_x0, phase_y_eulr, phase_y0, grid_pitch,
                                         correct_phase=False)

disp_x_lagr, _ = recolo.deflectomerty.disp_fields_from_phases(phase_x_lagr, phase_x0, phase_y_eulr, phase_y0, grid_pitch,
                                         correct_phase=False)
