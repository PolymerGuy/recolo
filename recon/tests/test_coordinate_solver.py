...
def disp_func(x, y):
    return (0.4 * np.cos(np.pi * x / 30) + 0.5 * np.sin(np.pi * y / 40)), (
            0.6 * np.cos(np.pi * x / 50) + 0.7 * np.sin(np.pi * y / 60))


class TestCoordinateSolver(TestCase):
    ...
    def test_compare_interpolated_and_analytical(self):
        ...
        if rms_diff_u_x > tol or rms_diff_u_y > tol:
            plt.imshow(np.abs(u_X-u_X_int),vmin=0,vmax=tol)
            plt.show()
