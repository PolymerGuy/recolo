    ...
    def test_additive_gaussian_noise(self):
        ...
        for noise_level in noise_levels:
            ...
            if np.abs(estimated_noise_level - noise_level) / noise_level > rel_error_tol:
                self.fail(
                    "Correct noise amplitude was %f but the estimated was %f" % (noise_level, estimated_noise_level))


class Test_DeflectometryOnKnownDeformationsGaussian(TestCase):
...
class Test_DeflectometryOnKnownDeformationsTriangular(TestCase):
    ...
    def test_rigid_body_motion_small_disp(self):
        ...
        disp_x_from_phase, disp_y_from_phase = disp_from_grids(grid_undeformed, grid_deformed, grid_pitch,
                                                               correct_phase=True, window="triangular")
    ...
    def test_sine_small_disp(self):
        ...
        u_X = u_X[grid_pitch-1:-grid_pitch+1,grid_pitch-1:-grid_pitch+1]
        u_Y = u_Y[grid_pitch-1:-grid_pitch+1,grid_pitch-1:-grid_pitch+1]
